...
extension SQLServerConstraintClient {
    ...
    @discardableResult
    internal func dropUniqueConstraint(name: String, table: String, schema: String = "dbo") -> EventLoopFuture<[SQLServerStreamMessage]> {
        let promise = client.eventLoopGroup.next().makePromise(of: [SQLServerStreamMessage].self)
        if #available(macOS 12.0, *) {
            promise.completeWithTask {
    ...
    @available(macOS 12.0, *)
    @discardableResult
    public func dropUniqueConstraint(name: String, table: String, schema: String = "dbo") async throws -> [SQLServerStreamMessage] {
        let escapedConstraintName = SQLServerSQL.escapeIdentifier(name)
        let escapedTableName = SQLServerSQL.escapeIdentifier(table)
        let schemaPrefix = schema != "dbo" ? "\(SQLServerSQL.escapeIdentifier(schema))." : ""
        let fullTableName = "\(schemaPrefix)\(escapedTableName)"
    ...
    @discardableResult
    internal func dropPrimaryKey(name: String, table: String, schema: String = "dbo") -> EventLoopFuture<[SQLServerStreamMessage]> {
        let promise = client.eventLoopGroup.next().makePromise(of: [SQLServerStreamMessage].self)
        if #available(macOS 12.0, *) {
            promise.completeWithTask {
    ...
    @available(macOS 12.0, *)
    @discardableResult
    public func dropPrimaryKey(name: String, table: String, schema: String = "dbo") async throws -> [SQLServerStreamMessage] {
        let escapedConstraintName = SQLServerSQL.escapeIdentifier(name)
        let escapedTableName = SQLServerSQL.escapeIdentifier(table)
        let schemaPrefix = schema != "dbo" ? "\(SQLServerSQL.escapeIdentifier(schema))." : ""
        let fullTableName = "\(schemaPrefix)\(escapedTableName)"
    ...
    @discardableResult
    internal func dropDefaultConstraint(name: String, table: String, schema: String = "dbo") -> EventLoopFuture<[SQLServerStreamMessage]> {
        let promise = client.eventLoopGroup.next().makePromise(of: [SQLServerStreamMessage].self)
        if #available(macOS 12.0, *) {
            promise.completeWithTask {
    ...
    @available(macOS 12.0, *)
    @discardableResult
    public func dropDefaultConstraint(name: String, table: String, schema: String = "dbo") async throws -> [SQLServerStreamMessage] {
        let escapedConstraintName = SQLServerSQL.escapeIdentifier(name)
        let escapedTableName = SQLServerSQL.escapeIdentifier(table)
        let schemaPrefix = schema != "dbo" ? "\(SQLServerSQL.escapeIdentifier(schema))." : ""
        let fullTableName = "\(schemaPrefix)\(escapedTableName)"
