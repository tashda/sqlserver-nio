...
final class SQLServerBatchTests: XCTestCase {
    ...
    func testExecuteScriptWithGOSeparators() async throws {
        ...
        // Verify data was inserted using SQLServerKit API
        let tableInfo = try await client.listTables(database: "master", schema: "dbo").get()
        XCTAssertTrue(tableInfo.contains { $0.name == tableName })

        // Get record count using proper query API
        let result = try await client.query("SELECT COUNT(*) as count FROM [\(tableName)]").get()
        XCTAssertEqual(result.first?.column("count")?.int, 2)

        // Cleanup using SQLServerKit API
    ...
    func testExecuteSeparateBatches() async throws {
        ...
        // Verify final state using SQLServerKit query API
    ...
    func testBatchWithTrigger() async throws {
        ...
        try await adminClient.dropTable(name: tableName)
        try await adminClient.dropTable(name: auditTableName)
    }
    ...
    func testBatchWithConstraints() async throws {
        ...
        // Cleanup
        try await adminClient.dropTable(name: childTableName)
        try await adminClient.dropTable(name: parentTableName)
    }
