...
final class SQLServerIndexTests: XCTestCase {
    ...
    func testCreateSimpleIndex() async throws {
        ...
        let result = try await client.query("SELECT * FROM [\(tableName)] WHERE name = N'John Doe'")
    ...
    func testCreateUniqueIndex() async throws {
        ...
        do {
            _ = try await client.execute("INSERT INTO [\(tableName)] (id, name, email, age, created_date) VALUES (6, N'Test User', N'john@example.com', 40, '2023-01-06 15:00:00')")
    ...
    func testDropIndex() async throws {
        ...
        // Verify it exists
        var exists = try await indexClient.indexExists(name: indexName, table: tableName)
        XCTAssertTrue(exists, "Index should exist after creation")

        // Drop the index
        try await indexClient.dropIndex(name: indexName, table: tableName)

        // Verify it's gone
        exists = try await indexClient.indexExists(name: indexName, table: tableName)
    ...
    func testReorganizeIndex() async throws {
        ...
        let result = try await client.query("SELECT * FROM [\(tableName)] WHERE name = N'Jane Smith'")
        XCTAssertEqual(result.count, 1)
    }
