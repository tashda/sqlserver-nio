    ...
    func testTableScriptGoldenRecreate() async throws {
        ...
        try await withTemporaryDatabase(client: self.client, prefix: "tsmx") { db in
            ...
            for combo in cases {
                ...
                // Sanity: verify columns count matches original
                let rows = try await queryInDb(client: self.client, database: db, "SELECT COUNT(*) AS cnt FROM sys.columns WHERE object_id = OBJECT_ID(N'dbo.\(table)')")
                XCTAssertEqual(rows.first?.column("cnt")?.int, combo.cols.count, "Column count mismatch after re-exec for \(table)")
