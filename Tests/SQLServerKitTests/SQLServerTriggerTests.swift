...
final class SQLServerTriggerTests: XCTestCase {
    ...
    override func tearDownWithError() throws {
        // Drop any triggers that were created during the test
        for trigger in triggersToDrop {
            let dropSql = "IF OBJECT_ID('[\(trigger.schema)].[\(trigger.name)]', 'TR') IS NOT NULL DROP TRIGGER [\(trigger.schema)].[\(trigger.name)]"
            do {
