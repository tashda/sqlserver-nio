...
final class SQLServerSecurityTests: XCTestCase {
    ...
    override func tearDownWithError() throws {
        ...
        // Drop any users that were created during the test
        for user in usersToDrop {
            let dropSql = "IF EXISTS (SELECT * FROM sys.database_principals WHERE name = '\(user)' AND type IN ('S', 'U', 'G')) DROP USER [\(user)]"
            do {
